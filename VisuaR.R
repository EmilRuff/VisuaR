...
# # Please cite https://sourceforge.net/projects/visuar/ or 
...
PathToVisuarOutput=file.path('C:/Users/User1/Desktop') # At this location the folder 'VisuaR' will be created containing subfolders with your analyses (named as provided in VisuaRProjectname)

# # Provide your dada2 output
PathToSeqtabNochim=file.path('C:/Users/User1/Desktop/FancyProject_seqtab_nochim.rds') # provide the location and name of your seqtab_nochim.rds
PathToTaxonomy=file.path('C:/Users/User1/Desktop/FancyProject_taxa_species.rds')      # provide the location and name of your taxonomy.rds

# # Analysis specifications
KingdomOfInterest='Bacteria'  # Which kingdom are you interested in? choose Bacteria/Archaea/Eukaryota
...
Metadatafilepath=file.path('C:/Users/User1/FancyProject_metadata.txt')  # provide the file path to your contextual data. i.e. 'C:/Users/Contextdata.txt' 
Grouping1=''                    # Enter column header as found in your contextual data where your discrete groups for the analysis are found. E.g. 'Biome'
...
cat('VISUAR Analysis',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\n\n1. User INPUT',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
sink(file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),append=TRUE,type='output')
cat('\n',sep='')
...
cat('\n\n2. Prologue',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

# #=== 2.3. Loads required packages =======================================================================================================================================================

cat('\n2.3. Load required packages',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\nInformation on your used package Versions and R version can be found in the txt file VersionInformation.txt',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
sink(file=(file.path(PathToVisuaRAnalysis,'VersionInformation.txt')),append=TRUE)
...
cat('\n\n2.4. Read and convert dada2 sequence table and taxonomy to match format used by VisuaR.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
...
if (identical(rownames(M.taxo.noNA),colnames(M.seqtab.nochim))){                # checks whether the ASVs in the taxonomy and in the seqtab_nochim are identical
  ...
  cat("\nError: You're Taxonomy file does not match your Seqtab.\n",file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\t',append=TRUE)
  stop("You're Taxonomy file does not match your Seqtab.")
}

ASV.mapfile=cbind(ASV.names,M.taxo.noNA)                                            # prints a mapfile to archive which ASV identifier (running number) belongs to which ASV sequence
...
write.table(ASV.mapfile,file.path(PathToVisuaRAnalysis,paste(VisuaRProjectName,'_ASV_Mapfile.txt',sep='')),sep='\t',col.names = NA)
...
M.reads.df=cbind('Sample Name'=rownames(M.reads.df),data.frame(M.reads.df,row.names = NULL)) 
colnames(M.reads.df)[2]='Reads'
write.table(M.reads.df,file.path(PathToVisuaRAnalysis,'Alpha_Diversity',paste(VisuaRProjectName,'_ReadsperSample_Original.txt',sep='')),sep='\t',col.names =NA)
if (SaveWholeworkspace=='N'){ rm(M.reads.df)}
...
M.differentASVsperSample=cbind(row.names(M.differentASVsperSample),data.frame(M.differentASVsperSample,row.names = NULL))
colnames(M.differentASVsperSample)[1]='Sample Name'
colnames(M.differentASVsperSample)[2]='ASV richness'
write.table(M.differentASVsperSample,file.path(PathToVisuaRAnalysis,'Alpha_Diversity',paste(VisuaRProjectName,'_ASVsperSample_Original.txt',sep='')),sep='\t',col.names = NA )
if (SaveWholeworkspace=='N') {rm(M.differentASVsperSample)}

# #=== 2.8. Excludes samples based on the MinimumAllowedReadCount ==========================================================================================================================
cat('\n2.8. Excludes samples based on read counts lower than ',MinimumAllowedReadCount,' (MinimumAllowedReadCount).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

if (MinimumAllowedReadCount==''|MinimumAllowedReadCount==0) {
  cat('\nNo samples were excluded based on their read counts as no MinimumAllowedReadCount was chosen.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
} else {
  ...
  if (ncol(M.seq.tax.MinimumAllowedReadCount.samples.excluded)==ncol.taxo.noNA) { # this is TRUE if all samples have read counts higher than MinimumAllowedReadCount
    cat('\nNo samples were excluded based on low read counts as all samples had more than ',MinimumAllowedReadCount,' reads.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  } else {
    ...
    cat(colnames(M.seq.tax.MinimumAllowedReadCount.samples.excluded[1:((ncol(M.seq.tax.MinimumAllowedReadCount.samples.excluded))-ncol.taxo.noNA)]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=TRUE)
    ...
    if (nrow(M.seq.tax.MinimumAllowedReadCount.ASVs.excluded)!=0) {
      ...
      cat(rownames(M.seq.tax.MinimumAllowedReadCount.ASVs.excluded),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    } else {
      cat('No ASVs have been completely excluded.\n',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
    }
  }
}
cat((ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
if (SaveWholeworkspace=='N') {rm(M.seq.tax.MinimumAllowedReadCount.ASVs.excluded,M.seq.tax.MinimumAllowedReadCount.samples.excluded)}

# #=== 2.9. Excludes ASVS not belonging to the KingdomofInterest ===========================================================================================================================

cat('\n\n2.9. Excludes ASVs not belonging to the chosen kingdom ',KingdomOfInterest,file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if (KingdomOfInterest=='') {
  ...
  cat(rownames(M.seq.tax.kingdom.ASV.excluded.org),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=', ',append=T)
  cat('\n',nrow(M.seq.tax.subset),' out of ',nrow(M.seq.tax),' ASVs remain belonging to the kingdom ',KingdomOfInterest,'.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  ...
  if (ncol(M.seq.tax.kingdom.samples.excluded)!=ncol.taxo.noNA&ncol(M.seq.tax.kingdom.samples.excluded.org)!=ncol.taxo.noNA) {                      # only happens if at least one sample was excluded in both dfs (M.seq.tax and M.seq.tax.subset)
    ...
    cat(colnames(M.seq.tax.kingdom.samples.excluded.org[c(1:(ncol(M.seq.tax.kingdom.samples.excluded.org)-ncol.taxo.noNA))]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append = T)
    ...
    cat(colnames(M.seq.tax.kingdom.samples.excluded[c(1:(ncol(M.seq.tax.kingdom.samples.excluded)-ncol.taxo.noNA))]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append = T)
...
cat((ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
if (SaveWholeworkspace=='N') {rm(M.seq.tax.kingdom.ASV.excluded,M.seq.tax.kingdom.ASV.excluded.org,M.seq.tax.kingdom.ASV.kept.org, M.seq.tax.kingdom.samples.excluded,M.seq.tax.kingdom.samples.excluded.org)}

# #=== 2.10. Keeps particular ASVs based on the clade they belong to ========================================================================================================================
cat('\n\n2.10. Keeps particular ASVs based on the clade they belong to',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

if (CladeOfInterest=='') {
  cat('\nNo Clade of Interest was chosen.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append = T)
} else {
  ...
  cat('\n',nrow(M.seq.tax.clade.ASV.excluded.org),' out of ',nrow(M.seq.tax),' ASVs have been excluded because they do not belong to the chosen clade.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  cat('\n',nrow(M.seq.tax.subset),' out of ',nrow(M.seq.tax),' ASVs remain belonging to the clade ',CladeOfInterest,' or where this expression occurs in the taxonomy.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  ...
  if (ncol(M.seq.tax.clade.samples.excluded)!=ncol.taxo.noNA&(ncol(M.seq.tax.clade.samples.excluded.org)!=ncol.taxo.noNA)) { # only happens if at least one sample was excluded in both dfs
    ...
    cat(colnames(M.seq.tax.clade.samples.excluded.org[c(1:(ncol(M.seq.tax.clade.samples.excluded.org)-ncol.taxo.noNA))]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    ...
    cat(colnames(M.seq.tax.clade.samples.excluded[c(1:(ncol(M.seq.tax.clade.samples.excluded)-ncol.taxo.noNA))]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    ...
    cat('\nNo samples were excluded due to no observed ASVs in the chosen clade.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  }
}
cat('\n',(ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)


# #=== 2.11. Excludes particular ASVs based on the clade they belong to =====================================================================================================================

cat('\n\n2.11. Excludes particular ASVs based on the clade they belong to.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

if (ExcludeClade=='') {
  cat('\nNo Clade to exclude was chosen.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append = T)
} else {
  ...
  cat('\n',nrow(M.seq.tax.clade2.ASV.excluded.org),' out of ',nrow(M.seq.tax),' ASVs have been excluded because they do belong to the clade ',ExcludeClade,'.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  cat('\n',nrow(M.seq.tax.subset),' out of ',nrow(M.seq.tax),' ASVs remain not belonging to the clade ',ExcludeClade,'.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  ...
  if (ncol(M.seq.tax.clade2.samples.excluded)!=ncol.taxo.noNA&ncol(M.seq.tax.clade2.samples.excluded.org)!=ncol.taxo.noNA) { # only happens if at least one sample was excluded in both dfs
    ...
    cat(colnames(M.seq.tax.clade2.samples.excluded.org[c(1:(ncol(M.seq.tax.clade2.samples.excluded.org)-ncol.taxo.noNA))]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    ...
    cat(colnames(M.seq.tax.clade2.samples.excluded[c(1:(ncol(M.seq.tax.clade2.samples.excluded)-ncol.taxo.noNA))]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    ...
    cat('\nNo samples were excluded due to no observed ASVs in the remaining clades.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  }
}
cat('\n',(ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
...
cat('\n\n2.12. Keeps particular samples based on the sample name.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

if (KeepSamplesbyName=='') {
  cat('\nNo particular samples to keep were chosen.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append = T)
} else {
  ...
  cat(KeepSamplesbyName,file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
  cat('The following ',ncol(M.seq.tax.keepsample.excluded.org)-ncol.taxo.noNA,' samples were excluded.\n',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  cat(colnames(M.seq.tax.keepsample.excluded.org[,1:(ncol(M.seq.tax.keepsample.excluded.org)-ncol.taxo.noNA)]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
  ...
  if (nrow(M.seq.tax.keepsample.ASVs.excluded)!=0&nrow(M.seq.tax.keepsample.ASVs.excluded.org)!=0) { # only happens if at least one ASV was excluded in both dfs
    ...
    cat(rownames(M.seq.tax.keepsample.ASVs.excluded.org),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=', ',append=T)
    ...
    cat(rownames(M.seq.tax.keepsample.ASVs.excluded),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=', ',append=T)
    ...
    cat('\nNo ASVs were excluded due to no observed ASVs in the remaining samples.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  }
}
cat('\n',(ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
...
cat('\n\n2.13. Excludes particular samples based on the sample name.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)

if (excludeSamplesbyName=='') {
  cat('\nNo particular samples to exclude were chosen.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append = T)
} else {
  ...
  cat(excludeSamplesbyName,file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
  cat('The following ',ncol(M.seq.tax.excludesample.excluded.org),' samples were excluded.\n',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  cat(colnames(M.seq.tax.excludesample.excluded.org),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
  ...
  if (nrow(M.seq.tax.excludesample.ASVs.excluded)!=0&nrow(M.seq.tax.excludesample.ASVs.excluded.org)!=0) { # only happens if at least one ASV was excluded in both dfs
    ...
    cat(rownames(M.seq.tax.excludesample.ASVs.excluded.org),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=', ',append=T)
...
cat('\n\n2.14. Excludes remaining samples based on read counts lower than ',MinimumAllowedReadCount.Analysis,' (MinimumAllowedReadCount.Analysis).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

if (MinimumAllowedReadCount.Analysis==''|MinimumAllowedReadCount.Analysis==0) {
  cat('\nNo samples were excluded based on their read counts as no MinimumAllowedReadCount.Analysis was chosen or it was set to 0.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
} else {
  ...
  if (ncol(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded)!=ncol.taxo.noNA&ncol(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded.org)!=ncol.taxo.noNA) { # only if at least one sample was excluded in both dfs
    ...
    cat(colnames(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded[,1:((ncol(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded))-ncol.taxo.noNA)]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    ...
    if (nrow(M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded)!=0&nrow(M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded.org)!=0) { # only happens if at least one ASV was excluded in both dfs
      ...
      cat(rownames(M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=', ',append=T)
      ...
      cat(rownames(M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded.org),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=', ',append=T)
      ...
      cat('\nNo ASVs were excluded because they do not occur in any of the samples anymore.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
    }
    ...
    cat(colnames(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded.org[,1:((ncol(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded.org))-ncol.taxo.noNA)]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='\n',append=T)
    M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded.org=M.seq.tax.MinimumAllowedReadCount.Analysis.samples.kept.org[rowSums(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.kept.org[,1:(ncol(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.kept.org)-ncol.taxo.noNA)])==0,] # creates an ASV by Sample&Taxonomy df with all ASVs which do not occur in the  samples (based on the original dataset (M.seq.tax)
    ...
    cat('\nNo samples were excluded based on low read counts as all samples had more than ',MinimumAllowedReadCount.Analysis,' reads.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
  }
}
cat('\n',(ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
if (SaveWholeworkspace=='N') {rm(M.seq.tax.MinimumAllowedReadCount.Analysis.samples.kept.org,M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded,M.seq.tax.MinimumAllowedReadCount.Analysis.samples.excluded.org,M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded.org,M.seq.tax.MinimumAllowedReadCount.Analysis.ASVs.excluded)}
...
cat(colnames(M.metadata),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep=", ",append=TRUE)
...
if (Metadata=='Y'&Grouping1!='NoMetadata') { # checks whether your selected Groupings do occur in your metadata file
  if(Grouping1 %in% colnames(M.metadata)) {
    if (Grouping2 %in% colnames(M.metadata)) {
      cat('\nYour chosen categories (Grouping1: ',Grouping1,', Grouping2: ',Grouping2,') do occur in your metadata file. Yeay!',file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="",append=TRUE)
    }
    else {
...
cat('\n\n2.16. Excludes Samples from your metadata sheet based on a category in your map file.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if (ColumnOfVariableToExclude!='') {
  ...
  if (nrow(M.metadata.CategoryToExcluded.samples.excluded)!=0) {                                                                        # Only happens if at least 1 sample was excluded
    ...
    cat(rownames(M.metadata.CategoryToExcluded.samples.excluded),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\n",append=TRUE)
...
cat('\n2.17. Excludes Samples based on selected grouping parameters.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\nYour samples will be grouped  based on values in the parameter ',Grouping1,' (Grouping1) and sorted inside these groups based on the parameter ',Grouping2,' (Grouping2).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if (nrow(M.metadata.subset.NA)==0){ 
  cat('\nNo sample was excluded from your contextual data file due to missing contextual data in the chosen groupings.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
} else {
  ...
  cat(rownames(M.metadata.subset.NA),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\n",append=TRUE)
  cat('No metadata in Grouping1: ',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
  cat(rownames(subset(M.metadata.subset,is.na(M.metadata.subset[,MapCol1]))),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep=", ",append=TRUE)
  cat('\nNo metadata in Grouping2: ',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
  cat(rownames(subset(M.metadata.subset,is.na(M.metadata.subset[,MapCol2]))),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep=", ",append=TRUE)
}
...
cat('\n\n2.18. Subsets Metadata to fit to your subsetted sequencing data (M.seq.tax.subset).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if (nrow(M.metadata.subset.noNA)!=nrow(M.metadata.subset)) { # only happens if at least one sample does not occur in the Metadata which does occur in M.seq.tax.subset and vice versa
  ...
  cat(rownames(M.metadata.subset.noseq),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\n",append=TRUE)
  if (SaveWholeworkspace=='N') {rm(M.metadata.subset.noseq,KeepRows)}
} else {
  cat('\nNo samples have been excluded from your metadata because all samples in your metadata occur in your M.seq.tax.subset.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
}
...
cat('\n\n2.19. Subsets M.seq.tax.subset to samples where metadata is available for the selected grouping parameters.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if(ncol(M.seq.tax.subset.noMetadata)==ncol.taxo.noNA){ # only happens if no samples was excluded (all samples occur in the subsetted metadatafile.)
  cat('\nNo samples have been excluded from your sequencing data because they do not occur in your metadata file (after subsetting).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
  ...
  cat(colnames(M.seq.tax.subset.noMetadata[,1:(ncol(M.seq.tax.subset.noMetadata)-ncol.taxo.noNA)]),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\n",append=TRUE)
  cat('\nThe following ',(ncol(M.seq.tax.subset)-ncol.taxo.noNA) ,' samples out of originally ' ,(ncol(M.seq.tax)-ncol.taxo.noNA),'samples remain for further analysis',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
}
...
cat('\n\n2.20. Creates Group and Color vectors for the visualization.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\nYour chosen groups subsetted for VisuaR contain in total ',length(M.projects),' samples.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
for (i in 1:length(M.projects.unique)) {
  cat('\nGroup ',M.group.count.ord[i,1],' contains ',M.group.count.ord[i,2],' samples and will appear colored in ',M.match.col.names[i,2],'.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
}
if (SaveWholeworkspace=='N') {rm(M.group.count,M.group.count.ord,M.match.col.names)}

# #=== 2.21. Subsampling finished ==========================================================================================================================================================

cat('\n\n2.21. The subsampling is finished.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
if (nrow(M.seq.tax.metadata.ASVs.excluded)!=0) { # Only happens if a sample was excluded and some of the ASVs now do not occur anymore
  ...
  cat(rownames(M.seq.tax.metadata.ASVs.excluded),file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep=",",append=TRUE)
} else {
  cat('\nNo ASVs have been excluded because they are no longer present in any of the samples.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
}
if (SaveWholeworkspace=='N') {rm(M.seq.tax,M.seq.tax.metadata.ASVs.excluded)}
cat('\nThe following',(ncol(M.seq.tax.subset)-ncol.taxo.noNA),' samples with a total of ',(nrow(M.seq.tax.subset)),' different ASVs out of originally ',nrow(M.seqtab.nochim),' samples with ',(ncol(M.seqtab.nochim)),' different ASVs remain for analysis.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
cat('\n',colnames(M.seq.tax.subset[,1:(ncol(M.seq.tax.subset)-ncol.taxo.noNA)]),file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep=',',append=T)
...
cat('\nThe remaining samples contain in total ',sum(M.sample.reads.left),' ASV counts. With an average of ',round(mean(M.sample.reads.left),1),',  a minimum of ',min(M.sample.reads.left),' and a maximum of ',max(M.sample.reads.left),' ASV counts.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
...
cat('\nThe remaining samples contain in total ',nrow(M.seq.tax.subset),' different ASVs. The samples contain on average ',round(mean(M.ASV.per.sample),1),', with a minimum of ',min(M.ASV.per.sample),' and a maximum of ',max(M.ASV.per.sample),' different ASVs.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)
...
cat('\nThe remaining ASVs occur on average ',round(mean(M.ASV.reads.left),1),' times, with a minimum of ',min(M.ASV.reads.left),' times, a maximum of ',max(M.ASV.reads.left),' times.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=T)

# #=== 3. Community Composition ============================================================================================================================

cat('\n\n\n3. Community Composition',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

# #=== 3.1. Creates relative abundance, presence absence tables ============================================================================================================================

cat('\n3.1. Creates presence/absence, relative abundance and other tables.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
write.table(M.seq.tax.subset.print,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_seqtax_subset.txt",sep="")),sep='\t',col.names = NA ) #write table
...
write.table(M.seq.tax.subset.rel,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_relabund_subset.txt",sep="")),sep="\t",col.names = NA)
...
write.table(M.seq.tax.subset.pa,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_pa_subset.txt",sep="")),sep="\t",col.names = NA)
if (SaveWholeworkspace=='N') {rm(M.ASV.pa,M.seq.tax.subset.ASVs,M.seq.tax.subset.pa,M.seq.tax.subset.tax)}

# # Calculates Reads per Sample after subsetting.
M.sample.reads.left=as.data.frame(M.sample.reads.left)
M.sample.reads.left=cbind('Sample Names'=rownames(M.sample.reads.left),data.frame(M.sample.reads.left,row.names = NULL))
colnames(M.sample.reads.left)[2]="Reads"
write.table(M.sample.reads.left,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_ReadsperSample_subset.txt",sep="")),sep='\t',col.names = NA )
if (SaveWholeworkspace=='N') {rm(M.sample.reads.left)}
...
write.table(M.ASV.reads.left,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_ReadsperASV_subset.txt",sep="")),sep='\t',col.names = NA )
if (SaveWholeworkspace=='N') {rm(M.ASV.reads.left)}

# # Calculates ASVs per Sample after subsetting.
M.ASV.per.sample=as.data.frame(M.ASV.per.sample)
M.ASV.per.sample=cbind('Sample Names'=rownames(M.ASV.per.sample),data.frame(M.ASV.per.sample,row.names = NULL))
colnames(M.ASV.per.sample)[2]="Number of different ASVs"
write.table(M.ASV.per.sample,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_ASVsperSample_subset.txt",sep="")),sep='\t',col.names = NA )
if (SaveWholeworkspace=='N') {rm(M.ASV.per.sample)}
...
M.seq.tax.subset.ord.ASV=M.seq.tax.subset.ord[,1:(ncol.M.seq.tax.subset-ncol.taxo.noNA)] # make ASV table without taxonomy, ordered based on taxonomy
...
M.seq.tax.subset.ord.print=cbind(row.names(M.seq.tax.subset.ord),data.frame(M.seq.tax.subset.ord,row.names = NULL)) # reformat table to save as .txt
colnames(M.seq.tax.subset.ord.print)[1]="ASV"
write.table(M.seq.tax.subset.ord.print,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_seqtax_subset_sort.txt",sep="")),sep='\t',col.names = NA ) #this table is sorted by groups and inside the groups based on the taxonomy on an alphabetical order
if (SaveWholeworkspace=='N') {rm(M.seq.tax.subset.ord.print)}

# #=== 3.3. Creates tables using individual taxonomic levels ===============================================================================================================================

cat('\n\n3.3. Creates tables using individual taxonomic levels',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\nThe tables will be saved to ',CompositionTablePath,'.\nFor each taxonomic level 4 tables will be created.\n1. ..TaxonomicLevelBySamples_seqtax.txt: A Taxonomy by sample table with all reads belonging to the clades summed up and the average of the clades in the last column.\n2. ..TaxonomicLevelBySamples_seqtax.sort.txt: A Taxonomy by sample table with all reads belonging to the clades summed up and the average of the clades in the last column. Depending on the value set in NumberOfTOPClades the rare taxa will be summed up in the last row and named as others. The Clades are ordered based on descending means.\n3. ..TaxonomicLevelBySamples_seqtax_ra.sort.txt: Same as 2. but wiht relative abundances instead of Reads.\n4. ..TaxonomicLevelBySamples_seqtax_pa.sort.txt: Same as 2. but wiht presences/absences instead of Reads.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
write.table(M.seq.tax.subset.phylum.sums,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_PhylaBySamples_seqtax.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.phylum.sums.ord,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_PhylaBySamples_seqtax_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.phylum.sums.ord.rel,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_PhylaBySamples_seqtax_ra_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.phylum.sums.ord.pa,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_PhylaBySamples_seqtax_pa_sort.txt",sep="")),sep='\t',col.names = NA )
...
# # Class level table
M.seq.tax.subset.class=M.seq.tax.subset.ord[,c(1:(ncol(M.seq.tax.subset)-ncol.taxo.noNA),(ncol(M.seq.tax.subset.ord)-ncol.taxo.noNA+3))]
...
write.table(M.seq.tax.subset.class.sums,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_ClassBySamples_seqtax.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.class.sums.ord,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_ClassBySamples_seqtax_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.class.sums.ord.rel,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_ClassBySamples_seqtax_ra_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.class.sums.ord.pa,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_ClassBySamples_seqtax_pa_sort.txt",sep="")),sep='\t',col.names = NA )
...
write.table(M.seq.tax.subset.order.sums,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_OrderBySamples_seqtax.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.order.sums.ord,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_OrderBySamples_seqtax_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.order.sums.ord.rel,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_OrderBySamples_seqtax_ra_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.order.sums.ord.pa,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_OrderBySamples_seqtax_pa_sort.txt",sep="")),sep='\t',col.names = NA )
...
write.table(M.seq.tax.subset.fam.sums,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_FamilyBySamples_seqtax.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.fam.sums.ord,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_FamilyBySamples_seqtax_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.fam.sums.ord.rel,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_FamilyBySamples_seqtax_ra_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.fam.sums.ord.pa,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_FamilyBySamples_seqtax_pa_sort.txt",sep="")),sep='\t',col.names = NA )
...
write.table(M.seq.tax.subset.genus.sums,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_GenusBySamples_seqtax.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.genus.sums.ord,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_GenusBySamples_seqtax_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.genus.sums.ord.rel,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_GenusBySamples_seqtax_ra_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.genus.sums.ord.pa,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_GenusBySamples_seqtax_pa_sort.txt",sep="")),sep='\t',col.names = NA )
...
write.table(M.seq.tax.subset.species.sums,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_SpeciesBySamples_seqtax.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.species.sums.ord,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_SpeciesBySamples_seqtax_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.species.sums.ord.rel,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_SpeciesBySamples_seqtax_ra_sort.txt",sep="")),sep='\t',col.names = NA )
write.table(M.seq.tax.subset.species.sums.ord.pa,file.path(CompositionTablePath,paste(VisuaRProjectName,"_",Grouping1,"_SpeciesBySamples_seqtax_pa_sort.txt",sep="")),sep='\t',col.names = NA )
...
cat('\n\n6.3. Create Bubble and BoxPlots of relative abundances for all taxonomic levels',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\nThe plots will be saved to ',CompositionPlotsPath,'.\nFor each taxonomic level a seperated folder will be created inside this folder.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\n1. The bubbleplot will show the samples on the x-axis (grouped and alphabetically sorted by ',Grouping1,' (Grouping1) and inside the groups sorted on increasing ',Grouping2,' (Grouping2)). The TOP',NumberOfTOPClades,' clades will be shown on the y axis. Relative Abundances will be presented as dot size.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\n2. The boxplot BoxByClade_RA will show the TOP',NumberOfTOPClades.box,' clades on the x-axis and the relative abundance on the y axis. For each Clade ',length(M.projects.unique),' boxplots will be shown (Number of your Groups provided as Grouping1).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\n3. The boxplot BoxByGroup_RA will show the ',length(M.projects.unique),' Groups provided as Grouping1 on the x axis and the relative abundance on the y axis. For each Clade 1 boxplot will be shown per Group.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\n4. ',NumberOfTOPClades,' Boxplots will be produced of the TOP',NumberOfTOPClades ,' clades. Clade_1 is the Clade with the highest abundance, Clade_2 the one with the second highest abundance and so on.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if (is.matrix(TVS)&is.matrix(TVS.b)) {
  ...
  if (is.matrix(TVG)&is.matrix(TVG.b)) {
      ...
      if (is.matrix(TVO)&is.matrix(TVO.b)) {
        ...
        if (is.matrix(TVC)&is.matrix(TVC.b)) {
          ...
          if (is.matrix(TVP)&is.matrix(TVP.b)) {
            startclade=1
            cat('\nPlots will be produced for all taxonomic levels.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
          } else {
            cat('\nNo plots will be produced on phylum level because only one phylum is present after subsetting (',as.character(M.seq.tax.subset.phylum[1,ncol(M.seq.tax.subset.phylum)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
          }
        } else {
          cat('\nNo plots will be produced on phylum level because only one phylum is present after subsetting (',as.character(M.seq.tax.subset.phylum[1,ncol(M.seq.tax.subset.phylum)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
          cat('\nNo plots will be produced on class level because only one class is present after subsetting (',as.character(M.seq.tax.subset.class[1,ncol(M.seq.tax.subset.class)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
        }
      } else {
        cat('\nNo plots will be produced on phylum level because only one phylum is present after subsetting (',as.character(M.seq.tax.subset.phylum[1,ncol(M.seq.tax.subset.phylum)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
        cat('\nNo plots will be produced on class level because only one class is present after subsetting (',as.character(M.seq.tax.subset.class[1,ncol(M.seq.tax.subset.class)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
        cat('\nNo plots will be produced on order level because only one order is present after subsetting (',as.character(M.seq.tax.subset.order[1,ncol(M.seq.tax.subset.order)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
      }
    } else {
      cat('\nNo plots will be produced on phylum level because only one phylum is present after subsetting (',as.character(M.seq.tax.subset.phylum[1,ncol(M.seq.tax.subset.phylum)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
      cat('\nNo plots will be produced on class level because only one class is present after subsetting (',as.character(M.seq.tax.subset.class[1,ncol(M.seq.tax.subset.class)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
      cat('\nNo plots will be produced on order level because only one order is present after subsetting (',as.character(M.seq.tax.subset.order[1,ncol(M.seq.tax.subset.order)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
      cat('\nNo plots will be produced on family level because only one family is present after subsetting (',as.character(M.seq.tax.subset.fam[1,ncol(M.seq.tax.subset.fam)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
    }
  } else {
    cat('\nNo plots will be produced on phylum level because only one phylum is present after subsetting (',as.character(M.seq.tax.subset.phylum[1,ncol(M.seq.tax.subset.phylum)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
    cat('\nNo plots will be produced on class level because only one class is present after subsetting (',as.character(M.seq.tax.subset.class[1,ncol(M.seq.tax.subset.class)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
    cat('\nNo plots will be produced on order level because only one order is present after subsetting (',as.character(M.seq.tax.subset.order[1,ncol(M.seq.tax.subset.order)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
    cat('\nNo plots will be produced on family level because only one family is present after subsetting (',as.character(M.seq.tax.subset.fam[1,ncol(M.seq.tax.subset.fam)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
    cat('\nNo plots will be produced on genus level because only one genus is present after subsetting (',as.character(M.seq.tax.subset.genus[1,ncol(M.seq.tax.subset.genus)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
  }
} else {
  startclade=7
  cat('\nNo plots will be produced because all levels contain only one clade after subsetting (',as.character(M.seq.tax.subset.phylum[1,ncol(M.seq.tax.subset.phylum)]),').',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
}
...
for (k in startclade:(ncol.taxo.noNA-1)){ 
  ...
  pdf(file.path(PathToVisuaRAnalysis,"Alpha_Diversity","CompositionPlots",tax.levels[k],paste(VisuaRProjectName,"BoxByGroup_RA.pdf",sep="")),height=(6),width=(NumberOfTOPClades*0.5)+((k-1)*2.5),useDingbats=F) #height=(8),width=(25)
...
cat('\n\n.4. Alpha Diversity',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
write.table(M,file.path(PathToVisuaRAnalysis,'Beta_Diversity',paste(VisuaRProjectName,"_M.txt",sep="")),sep='\t',col.names = NA) 
...
cat('\n4.1. Calculates diversity indices',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\nThe alpha diversity indices will be calculated with a subsample of ',M.minreads,' ASVs (minimum number of reads occuring in any sample).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\nObserved ASV richness, Chao1 richness estimate, Shannon Entropy, Inverse Simpson Diversity, absolute and relative Singletons can be found in the file ',file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_DiversityIndices_",Grouping1,".txt",sep="")),'.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\nNote: diversity indices may be based on iterative subsampling, whereas SSOabs and SSOrel are based on the whole dataset.\nThis can result in the odd case that there seem to be more observed relative ASVs than observed ASVs.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
####? überarbeiten: M.SSASV.abs.pc wird in erster if schleife nicht erstellt, überprüfen!
...
if (SaveWholeworkspace=='N') {rm(M.sobs,M.no.SSASVs.abs,M.SSASVs.rel,M.SSASVs.rel.nr,M.SSASVs.rel.pc,M.SSASVs.rel.name)}
...
M.diversity.reord=t(rbind(M.diversity[1:4,],M.diversity[7,],M.diversity[5,],M.diversity[8,],M.diversity[6,],M.diversity[9,]))                                   
M.diversity.reord[,c(1:4,6,8)]=round(M.diversity.reord[,c(1:4,6,8)],digits=0)                                                                                   # rounds indices
M.diversity.reord[,c(5,7,9)]=round(M.diversity.reord[,c(5,7,9)],digits=1)                                                                                       # rounds indices
colnames(M.diversity.reord)=c("Total Reads","Observed ASVs","Estimated ASVs (Chao1)","Inverse Simpson Diversity","Shannon Entropy","Absolute Single Sequence ASVs","Relative Single Sequence ASVs","Absolute Single Sequence ASVs (%)","Relative Single Sequence ASVs (%)")
write.table(M.diversity.reord,file.path(PathToVisuaRAnalysis,"Alpha_Diversity",paste(VisuaRProjectName,"_DiversityIndices_",Grouping1,".txt",sep="")),sep='\t',col.names = NA)

if (SaveWholeworkspace=='N') {rm(M.ASVs,M.SSASV.abs.nr.t,M.SSASV.abs.pc.t,M.SSASVs.rel.nr.t,M.SSASVs.rel.pc.t)}
if (SaveWholeworkspace=='N') {rm(M.diversity.reord)}

# #===  4.2. Visualizes diversity indices ================================================================================================================================================

cat('\n\n4.2. Visualizes diversity indices',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

# #===  4.2.1. Boxplots of diversity indices ================================================================================================================================================

cat('\n\n4.2.1. Boxplots of diversity indices',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\n\n4.2.2. Violinplots of diversity indices',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\n\n4.3. Species accumulation curves',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)

# #------- 4.3.1. Calculates Species accumulation curve using samples -----------------------------------------------------------------------------------------------------------------

cat('\n4.3.1. Calculates Species accumulation curve using samples',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\n4.3.2. Calculate Species accumulation curve using ',Grouping1,' (Grouping1).',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\n\n5. Beta Diversity',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\n5.1.  Anaylsis of Similarities (ANOSIM) and calculation of Non-metric Multi-Dimensional Scaling (NMDS) ordinations.',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
if(length(M.projects.unique)>1){
  ...
  dev.off()
  cat('\n6.5.1.1. Analysis of Similarities (ANOSIM)',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
  cat("\nAnosim_Dissimilarity: ",M.ano$dissimilarity,file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep='',append=TRUE)
  cat("\nAnosim_statistic_R: ",M.ano$statistic,file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep='',append=TRUE)
  cat("\nAnosim_Significance: ",M.ano$signif,file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep='',append=TRUE)
  cat("\nAnosim_Permutations: ",M.ano$permutations,file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep='',append=TRUE)
}

if (SaveWholeworkspace=='N') {rm(M.rel)}

# #--- 5.1.2. NMDS Calculation And Visualization ----------------------------------------------------------------------------------------------------------------------------------------
cat('\n\n5.1.2. NMDS calculation and Visualization',file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\nNote: Using metaMDS with a distance matrix is the same as using monoMDS without a distance matrix. You create a distance matrix using vegdist(method=bray) and apply metaMDS to this distrance matrix.' ,file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
cat('\nNote: Maximum, Mean and Minimal Community Similarity in % (as shown in the NMDS plots) refers to the comparison of any two samples using the bray-curtis distance matrix.' ,file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
...
saveRDS(M.mMDS,file.path(PathToVisuaRAnalysis,'Beta_Diversity',paste(VisuaRProjectName,"_MmMDS.rds",sep="")))
cat("\nNote: In NMDS ordinations the distance between two samples represents the distance of their underlying communities.\nNMDS distances are relative measures and thus, do not need an axis.\nAxes in NMDS ordinations are only used when NMDS is combined (i.e. superimposed on the NMDS ordination) with another method.",file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\t",append=TRUE)

M.mMDS.stress=M.mMDS$stress 
cat('\nNMDS_Stress_Level: ',M.mMDS.stress,file=(file.path(PathToVisuaRAnalysis,'Analysis_log.txt')),sep='',append=TRUE)
cat('\nNote: Stress values are a measure for the goodness of the NMDS (i.e. how realistic is the n-dimensional space represented in the 2-dimensional plot).',file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\t",append=TRUE)
cat('\nNote: R and p values shown in the NMDS plots are calculated using ANOSIM. Please consider limitations and interpretation of the ANOSIM test.',file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\t",append=TRUE)
cat('\nNote: NMDS featuring text of the samples is meant for reference and to look up specific samples yet is not optimized for display.',file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\t",append=TRUE)
...
cat('\nNote: Ordispider: each Sample is connected to the weighted average mean of the within group distances (centroid).',file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\t",append=TRUE)
cat('\nNote: Ordiellipse: visualizes one standard deviation from the groups centroids. ',file=(file.path(PathToVisuaRAnalysis,"Analysis_log.txt")),sep="\t",append=TRUE)
...
if (SaveWholeworkspace=='N') {rm(M.ano,M.mMDS)}
...
if (SaveWholeworkspace=='N') {rm(M.grouped,M.grouped.dist,M.grouped.clust,M.grouped.cd,M.grouped.phylo)}

# #------- 5.4. Calculates and visualizes Cluster Dendrograms by samples -----------------------------------------------------------------------------------------------------------------
#? Größe mit anzahl samploes skalieren, und außen nichts abschneiden
...
if (SaveWholeworkspace=='N') {rm(M.sample.dist,M.sample.clust,M.sample.clust,M.sample.phylo)}
...
if (SaveWholeworkspace=='N') {rm(M.up,M.upset)}
...
write.table(percentage,file.path(PathToVisuaRAnalysis,'Beta_Diversity',paste(VisuaRProjectName,"_Percent_Shared_ASVs_between_Groups.txt",sep="")),sep='\t',col.names = NA)
